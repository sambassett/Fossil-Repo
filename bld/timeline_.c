#line 1 "../fossil-1.32/src/timeline.c"
/*
...
void hyperlink_to_uuid(const char *zUuid){
  if( g.perm.Hyperlink ){
    cgi_printf("%z[%S]</a>\n",(xhref("class='timelineHistLink'","%R/info/%!S",zUuid)),(zUuid));
  }else{
...
void hyperlink_to_user(const char *zU, const char *zD, const char *zSuf){
  ...
  if( g.perm.Hyperlink ){
    if( zD && zD[0] ){
      cgi_printf("%z%h</a>%s\n",(href("%R/timeline?c=%T&u=%T",zD,zU)),(zU),(zSuf));
    }else{
      cgi_printf("%z%h</a>%s\n",(href("%R/timeline?u=%T",zU)),(zU),(zSuf));
    }
  }else{
    cgi_printf("%s\n",(zU));
  }
...
void test_hash_color_page(void){
  ...
  for(i=cnt=0; i<10; i++){
    ...
    if( zBr && zBr[0] ){
      cgi_printf("<p style='border:1px solid;background-color:%s;'>\n"
             ...
             "suam declinavit.</p>\n",(hash_color(zBr)),(zBr),(hash_color(zBr)));
      cnt++;
    }
  }
  if( cnt ){
  ...
  cgi_printf("<form method=\"post\" action=\"%s/hash-color-test\">\n"
...
void www_print_timeline(
  ...
  if( tmFlags & TIMELINE_GRAPH ){
    ...
    */
    cgi_printf("<div id=\"canvas\" style=\"position:relative;height:0px;width:0px;\"\n"
  ...
  while( db_step(pQuery)==SQLITE_ROW ){
    ...
    if( pendingEndTr ){
      cgi_printf("</td></tr>\n");
      if( pendingEndTr>1 ){
        cgi_printf("<tr class=\"timelineSpacer\"></tr>\n");
      }
      pendingEndTr = 0;
    }
    ...
    */
    ...
    cgi_printf("<td class=\"timelineTime\">%s</td>\n"
   ...
   cgi_printf("</td>\n");
    ...
    if( zType[0]!='c' ){
      ...
      ** "fossil rebuild" and expect to be rendered as text/x-fossil-wiki */
      ...
      cgi_printf("<span class=\"timelineComment\">%W</span>\n",(blob_str(&truncated)));
    ...
    */
    ...
    if( g.perm.Hyperlink && fossil_strcmp(zDispUser, zThisUser)!=0 ){
      ...
      cgi_printf("(user: %z%h</a>%s\n",(href("%z",zLink)),(zDispUser),(zTagList?",":"\051"));
    }else{
      cgi_printf("(user: %h%s\n",(zDispUser),(zTagList?",":"\051"));
    }
    ...
    if( (tmFlags & (TIMELINE_FCHANGES|TIMELINE_FRENAMES))!=0
     && zType[0]=='c' && g.perm.Hyperlink
      ...
      while( db_step(&fchngQuery)==SQLITE_ROW ){
        ...
        if( isNew ){
          cgi_printf("<li> %s%h</a>%s %s (new file) &nbsp;\n"
                 "%z[view]</a></li>\n",(zA),(zFilename),(zId),(zUnpub),(href("%R/artifact/%!S",zNew)));
        }else if( isDel ){
          ...
          cgi_printf("%z[diff]</a></li>\n",(href("%R/fdiff?sbs=1&v1=%!S&v2=%!S",zOld,zNew)));
        }
        fossil_free(zA);
      }
      db_reset(&fchngQuery);
    ...
    if( pGraph->nErr ){
      ...
      */
      ...
      cgi_printf("<tr><td></td><td>\n"
             "<div id=\"grbtm\" style=\"width:%dpx;\"></div>\n"
             "</td><td></td></tr>\n",(w));
    }
  }
  cgi_printf("</table>\n");
...
void timeline_output_graph_javascript(
  ...
  if( pGraph && pGraph->nErr==0 && pGraph->nRow>0 ){
    ...
    */
    ...
    graph_free(pGraph);
    cgi_printf("var cDiv = gebi(\"canvas\");\n"
    ...
    if( !omitDescenders ){
      cgi_printf("  if( p.u==0 ) drawUpArrow(p.x, 0, p.y-5);\n"
             "  if( p.d ) drawUpArrow(p.x, p.y+6, btm);\n");
    }
    cgi_printf("  if( p.mo>0 ){\n"
           "    var x1 = p.mo + left - 1;\n"
           "    var y1 = p.y-3;\n"
    ...
    cgi_printf("    drawThinLine(x1,y0,x1,y1);\n"
           "  }\n"
           "  var n = p.au.length;\n"
           "  for(var i=0; i<n; i+=2){\n"
           ...
           "  }\n");
    if( P("clicktest")!=0 ){
      cgi_printf("alert(\"click at \"+x+\",\"+y)\n");
    }
    cgi_printf("  for(var i in rowinfo){\n"
           "    p = rowinfo[i];\n"
           ...
           "  }else{\n");
    if( fileDiff ){
      cgi_printf("    location.href=\"%R/fdiff?v1=\"+selRow.h+\"&v2=\"+p.h+\"&sbs=1\";\n");
    }else{
      if( db_get_boolean("show-version-diffs", 0)==0 ){
        cgi_printf("    location.href=\"%R/vdiff?from=\"+selRow.h+\"&to=\"+p.h+\"&sbs=0\";\n");
      }else{
        cgi_printf("    location.href=\"%R/vdiff?from=\"+selRow.h+\"&to=\"+p.h+\"&sbs=1\";\n");
      }
    }
    cgi_printf("  }\n"
           "}\n"
...
void page_timeline(void){
  ...
  */
  ...
  db_finalize(&q);
  if( zOlderButton ){
    cgi_printf("%zOlder</a>\n",(xhref("class='button'","%z",zOlderButton)));
  }
  style_footer();
}
...
void test_timewarp_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<li>\n"
           "<a href=\"%R/timeline?dp=%!S&amp;unhide\">%S</a>\n",(zUuid),(zUuid));
  }
  db_finalize(&q);
  style_footer();
}
