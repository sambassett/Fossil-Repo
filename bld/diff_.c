...
void annotation_page(void){
  ...
  if( showLog ){
    ...
    cgi_printf("<h2>Ancestors of %z%h</a> analyzed:</h2>\n"
           "<ol>\n",(zLink),(zFilename));
    for(p=ann.aVers, i=0; i<ann.nVers; i++, p++){
      ...
      if( i>0 ){
        ...
        cgi_printf("%z[diff-to-top]</a>\n",(zLink));
        if( i>1 ){
           zLink = xhref("target='infowindow'",
                         "%R/fdiff?v1=%S&v2=%S&sbs=1",
                         p->zFUuid,p[-1].zFUuid);
           cgi_printf("%z[diff-to-previous]</a>\n",(zLink));
        }
      }
#endif
    }
  ...
  if( !ann.bLimit ){
    cgi_printf("<h2>Origin for each line in\n"
           "%z%h</a>\n"
           "from check-in %z%S</a>:</h2>\n",(href("%R/finfo?name=%h&ci=%!S", zFilename, zCI)),(zFilename),(href("%R/info/%!S",zCI)),(zCI));
    iLimit = ann.nVers+10;
  }else{
    cgi_printf("<h2>Lines added by the %d most recent ancestors of\n"
           "%z%h</a>\n"
           "from check-in %z%S</a>:</h2>\n",(iLimit),(href("%R/finfo?name=%h&ci=%!S", zFilename, zCI)),(zFilename),(href("%R/info/%!S",zCI)),(zCI));
  }
  cgi_printf("<pre>\n");
