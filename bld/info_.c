  ...
  if( !g.perm.Hyperlink ){
    ...
    if( zOld && zNew ){
      ...
      cgi_printf("<p>Deleted %z%h</a>\n"
             "version %z[%S]</a>\n",(href("%R/finfo?name=%T",zName)),(zName),(href("%R/artifact/%!S",zOld)),(zOld));
    }else{
...
void ci_page(void){
  ...
  if( db_step(&q1)==SQLITE_ROW ){
    ...
    if( g.perm.Hyperlink ){
      ...
      cgi_printf("</td></tr>\n"
             ...
             "  | %zfile ages</a>\n"
             "  | %zfolders</a>\n"
             "  | %zmanifest</a>\n",(href("%R/tree?ci=%!S",zUuid)),(href("%R/fileage?name=%!S",zUuid)),(href("%R/tree?nofiles&type=tree&ci=%!S",zUuid)),(href("%R/artifact/%!S",zUuid)));
      if( g.anon.Write ){
  ...
  if( verboseFlag ){
    cgi_printf("%z\n"
           "Hide&nbsp;Diffs</a>\n",(xhref("class='button'","%R/%s/%T",zPageHide,zName)));
    if( sideBySide ){
      cgi_printf("%z\n"
             "Unified&nbsp;Diffs</a>\n",(xhref("class='button'","%R/%s/%T?sbs=0%s",zPage,zName,zW)));
    }else{
      cgi_printf("%z\n"
             "Side-by-Side&nbsp;Diffs</a>\n",(xhref("class='button'","%R/%s/%T?sbs=1%s",zPage,zName,zW)));
    }
    if( *zW ){
      cgi_printf("%z\n"
             "Show&nbsp;Whitespace&nbsp;Changes</a>\n",(xhref("class='button'","%R/%s/%T?sbs=%d",zPage,zName,sideBySide)));
    }else{
      cgi_printf("%z\n"
             "Ignore&nbsp;Whitespace</a>\n",(xhref("class='button'","%R/%s/%T?sbs=%d&w",zPage,zName,sideBySide)));
    }
  }else{
    cgi_printf("%z\n"
           "Show&nbsp;Unified&nbsp;Diffs</a>\n"
           "%z\n"
           "Show&nbsp;Side-by-Side&nbsp;Diffs</a>\n",(xhref("class='button'","%R/%s/%T?sbs=0",zPage,zName)),(xhref("class='button'","%R/%s/%T?sbs=1",zPage,zName)));
  }
  if( zParent ){
    cgi_printf("%z\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( g.perm.Hyperlink ){
      cgi_printf("%z[ancestry]</a>\n"
             ...
             "[annotate]</a>\n"
             "%z\n"
             "[blame]</a>\n",(href("%R/finfo?name=%T&ci=%!S",zName,zVers)),(href("%R/annotate?filename=%T&checkin=%!S",zName,zVers)),(href("%R/blame?filename=%T&checkin=%!S",zName,zVers)));
    }
    cnt++;
