#line 1 "../fossil-1.32/src/cache.c"
/*
...
void cache_page(void){
  ...
  db = cacheOpen(0);
  if( db==0 ){
    cgi_printf("The web-page cache is disabled for this repository\n");
  }else{
    char *zDbName = cacheName();
    cache_register_sizename(db);
    pStmt = cacheStmt(db,
    ...
    if( pStmt ){
      ...
      while( sqlite3_step(pStmt)==SQLITE_ROW ){
        ...
        cgi_printf("<li><p>%z%h</a><br>\n"
               "size: %s\n"
               "hit-count: %d\n"
               "last-access: %s</p></li>\n",(href("%R/cacheget?key=%T",zName)),(zName),(sqlite3_column_text(pStmt,1)),(sqlite3_column_int(pStmt,2)),(sqlite3_column_text(pStmt,3)));
      }
      sqlite3_finalize(pStmt);
      cgi_printf("</ol>\n");
    }
    ...
    cgi_printf("<p>cache-file name: %h</p>\n"
           "<p>cache-file size: %s</p>\n",(zDbName),(zBuf));
    fossil_free(zDbName);
    sqlite3_close(db);
  }
  style_footer();
