...
void bloblist_page(void){
  ...
  cgi_printf("<table cellpadding=\"0\" cellspacing=\"0\">\n");
  while( db_step(&q)==SQLITE_ROW ){
    int rid = db_column_int(&q,0);
    const char *zUuid = db_column_text(&q, 1);
    ...
    cgi_printf("<tr><td align=\"right\">%d</td>\n"
           "<td>&nbsp;%z%s</a>&nbsp;</td>\n"
           "<td align=\"left\">%h</td>\n",(rid),(href("%R/info/%!S",zUuid)),(zUuid),(zDesc));
    if( isPriv ){
      cgi_printf("<td>(unpublished)</td>\n");
    }
    cgi_printf("</tr>\n");
  }
  cgi_printf("</table>\n");
