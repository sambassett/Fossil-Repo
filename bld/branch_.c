#line 1 "../fossil-1.32/src/branch.c"
/*
...
static void new_brlist_page(void){
  ...
  cgi_printf("<div class=\"brlist\"><table id=\"branchlisttable\">\n"
         "<thead><tr>\n"
         "<th>Branch Name</th>\n"
         "<th>Age</th>\n"
         "<th>Check-ins</th>\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           ...
           "<td>%d</td>\n",(href("%R/timeline?n=100&r=%T",zBranch)),(zBranch),(-iMtime),(zAge),(nCkin));
    fossil_free(zAge);
    cgi_printf("<td>%s</td>\n",(isClosed?"closed":""));
    if( zMergeTo ){
...
void brlist_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( colorTest ){
      ...
      cgi_printf("<li>%z%h</a></li>\n",(href("%R/timeline?r=%T&n=200",zBr)),(zBr));
    }
  }
  if( cnt ){
    cgi_printf("</ul>\n");
...
static void brtimeline_extra(int rid){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("%z[timeline]</a>\n",(href("%R/timeline?r=%T&n=200",zTagName)));
  }
  db_finalize(&q);
}
