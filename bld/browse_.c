#line 1 "../fossil-1.32/src/browse.c"
/*
  ...
  */
  ...
  if( zCI ){
    cgi_printf("<h2>Files of check-in [%z%S</a>]\n"
           "%s</h2>\n",(href("vinfo?name=%!S",zUuid)),(zUuid),(blob_str(&dirname)));
...
void fileage_page(void){
  ...
  cgi_printf("<h2>Files in\n"
         "%z[%S]</a>\n",(href("%R/info/%!S",zUuid)),(zUuid));
  if( zGlob && zGlob[0] ){
    cgi_printf("that match \"%h\" and\n",(zGlob));
  }
  cgi_printf("ordered by check-in time</h2>\n"
         "\n"
         ...
         "<tr><th>Time</th><th>Files</th><th>Check-in</th></tr>\n",(href("%R/ci/%!S",zUuid)),(zUuid),(href("%R/timeline?c=%t",zNow)),(zNow));
  db_prepare(&q1,
  ...
  while( db_step(&q1)==SQLITE_ROW ){
    ...
    while( db_step(&q2)==SQLITE_ROW ){
      ...
      if( showId ){
        cgi_printf("%z%h</a> (%d)<br>\n",(href("%R/artifact/%!S",zFUuid)),(zFile),(fid));
      }else{
        cgi_printf("%z%h</a><br>\n",(href("%R/artifact/%!S",zFUuid)),(zFile));
      }
    }
    db_reset(&q2);
    cgi_printf("</td>\n"
    ...
    cgi_printf("%W (user:\n"
           ...
           "%z%h</a>)\n"
           "</td></tr>\n"
           "\n",(zComment),(href("%R/timeline?u=%t&c=%!S&nd&n=200",zUser,zUuid)),(zUser),(href("%R/timeline?r=%t&c=%!S&nd&n=200",zBranch,zUuid)),(zBranch));
    fossil_free(zAge);
  }
